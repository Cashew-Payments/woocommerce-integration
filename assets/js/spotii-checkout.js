...
function render() {
  ((container = document.createElement("div")).className = "spotii-popup"),
    ...
    (closeBtn = renderCloseButton()).addEventListener("click", hide, !1),
...
function renderCloseButton() {
  var o = document.createElement("div");
  return (
    (o.className = "spotii-popup__close"),
    (o.innerHTML =
...
jQuery(document).ready(function (o) {
  ...
  (window.closeIFrameOnCompleteOrder = function ({ status: e }) {
    if ((console.log("Order state - ", e), first)) {
      ...
      api = o.cookie("api");
      switch (
        (o.removeCookie("total"),
        o.removeCookie("curr"),
        o.removeCookie("api"),
        o(".sptii-overlay").remove(),
        o(".sptii-content").remove(),
        e)
      ) {
        case "SUCCESS":
        ...
        case "FAILED":
          var errorMessage =
            ...
            "ar"
              ? "لقد حصل خطأ عند الدفع عن طريق سبوتي، رجاءً حاول مرة اخرى"
              : "Payment with Spotii failed. Please try again";
          spottiCapture(t, "cancelled", i, n, api),
            submit_error('<div class="woocommerce-error"></div>');
      }
    }
  }),
