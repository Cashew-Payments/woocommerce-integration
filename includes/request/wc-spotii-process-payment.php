...
function processPayment($order_id, $th, $type = null, $addon){
    ...
    try {
        ...
        if (array_key_exists('token', $response_body_arr['data'])) {
            ...
            $order->save();
            return array('result' => 'success', 'redirect' => "", 'token' => $response_body_arr['data']['token'], 'storeToken' => $th->token, "checkout_url" => $redirect_url, "orderId" => $response_body_arr['data']['orderId'], "api" => $th->api,"cancelURL" => $order->get_cancel_order_url(), "sucessURL" => '/checkout/order-received/'.$order_id.'/?key='.$order->get_order_key());
        } else {
