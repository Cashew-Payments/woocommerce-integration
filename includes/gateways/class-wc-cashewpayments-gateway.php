...
/**
 * Cashewpayments gateway abstract class.
 */
abstract class WC_Cashewpayments_Gateway extends WC_Payment_Gateway {

	/**
	 * Cashewpayments adapter instance.
	 *
	 * @var WC_Cashewpayments_Adapter
	 */
	protected $adapter;
	...
	public function __construct() {
		...
		require_once WC_CASHEWPAYMENTS_DIR_PATH . 'includes/class-wc-cashewpayments-api.php';

		new WC_Cashewpayments_Api( $this->id );
		$this->adapter = new WC_Cashewpayments_Adapter( $this );
	...
	public function payment_fields() {
		...
		wc_get_cashewpayments_template( 'payment-fields.php', array( 'gateway' => $this ) );
	}
	...
	public function process_payment( $order_id ) {
		...
		if ( $this->in_context ) {
			...
			if ( $this->sandbox ) {
				$host = 'checkout-sandbox.cashewpayments.io';
			} else {
				$host = 'checkout.cashewpayments.io';
			}
	...
	public function capture( $transaction_id, $order_key ) {
			...
			try {
				...
				$order->add_order_note(
					sprintf( /* translators: %1$s: transaction id %2$s: error code */
						__( 'Cashewpayments capture failed. ID: %1$s. Code: %2$s.', 'cashewpayments' ),
						$transaction_id,
						$e->getErrorCode()
					)
				);
		...
		wc_add_notice( __( 'Cashewpayments capture error.', 'cashewpayments' ), 'error' );
	...
	public function cancel( $transaction_id, $order_key ) {
			...
			if ( 'CANCELLED' !== $status ) {
				$order->add_order_note(
					sprintf( /* translators: %1$s: order status %2$s: transaction id */
						__( 'Cashewpayments order %1$s. ID: %2$s.', 'cashewpayments' ),
						strtolower( $status ),
						$transaction_id
					)
				);
		...
		wc_add_notice( __( 'Cashewpayments order cancelled.', 'cashewpayments' ) );
		wp_safe_redirect( wc_get_checkout_url() );
	}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		try {
			...
			return new WP_Error(
				'error',
				sprintf( /* translators: %1$s: transaction id %2$s: error code */
					__( 'Cashewpayments refund failed. ID: %1$s. Code: %2$s.', 'cashewpayments' ),
					$transaction_id,
					$e->getErrorCode()
				)
			);
