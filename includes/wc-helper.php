...
function wc_cashew_scripts()
{
    ...
    if (is_checkout()) {
        wp_enqueue_script('cashew-checkout', 'https://'.$domain.'.cashewpayments.com/widgets/woocommerce.checkout.min.js', array('jquery'), '1.0.0', true);
    }
    wp_enqueue_script('cashew-helper-checkout', $assetsPath . '../assets/js/cashew-checkout.js', array('jquery'), '1.0.0', true);
    if (is_checkout()) {
        wp_deregister_script('wc-checkout');
        wp_enqueue_script('wc-checkout', $assetsPath . '../assets/js/woocommerce-checkout.js', array('jquery'), '1.0.0', true);
    }
